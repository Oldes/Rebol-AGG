...
//
// 2026-01-21 Oldes
//   https://github.com/siskin-framework/agg
//
//----------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// Fill all with current fill style
void Agg2D::fillAll()
{
    Agg2D::Transformations saved;
    saved = transformations();  // Store current matrix.
    resetTransformations();
    rectangle(0, 0, m_renBase.width(), m_renBase.height());
    double w = m_renBase.width();
    double h = m_renBase.height();
    m_path.remove_all();
    m_path.line_to(w, 0);
    m_path.line_to(w, h);
    m_path.line_to(0, h);
...
//------------------------------------------------------------------------
void Agg2D::invertTransformations()
{
    m_transform.invert();
}
...
// Pre-multiply:
void Agg2D::pre_translate(double x, double y) { m_transform.premultiply(agg::trans_affine_translation(x, y)); }
void Agg2D::pre_rotate(double angle)          { m_transform.premultiply(agg::trans_affine_rotation(angle));   }
void Agg2D::pre_skew(double sx, double sy)    { m_transform.premultiply(agg::trans_affine_skewing(sx, sy));   }
void Agg2D::pre_scale(double sx, double sy)
{
    m_transform.premultiply(agg::trans_affine_scaling(sx, sy));
    m_convCurve.approximation_scale(worldToScreen(1.0) * g_approxScale);
    m_convStroke.approximation_scale(worldToScreen(1.0) * g_approxScale);
}
...
//------------------------------------------------------------------------
void Agg2D::fillImage(unsigned char* buf, int width, int height, int stride)
{
    m_img_fill.attach(buf, width, height, stride);
    m_fillType = Agg2D::RTImage;
    m_hasFill = true;
}
...
void Agg2D::drawPath(DrawPathFlag flag)
{
    ...
    if (m_fillType == Agg2D::RTImage && (flag == FillOnly || flag == FillAndStroke)) {
    ...
    {
    case FillOnly:
        if (m_hasFill && m_fillType == Agg2D::RTColor)
        {
            m_rasterizer.add_path(m_pathTransform);
            render(true);
        }
...
void Agg2D::pushState() {
    ...
    s.strokeType = m_strokeType;
    s.fillType = m_fillType;
}
...
void Agg2D::popState() {
    if (!m_stateStack.empty()) {
        ...
        m_strokeType = s.strokeType;
        m_fillType = s.fillType;
    }
}
...
if(fillColor && gr.m_fillType == Agg2D::RTImage) {
    gr.m_rasterizer.add_path(gr.m_pathTransform);  // Add path outline to rasterizer FIRST
    
    double parl[6];
    gr.m_transform.store_to(parl);  // Screen transform for image coords
    
    gr.renderImage(gr.m_img_fill, 0, 0, gr.m_img_fill.width(), gr.m_img_fill.height(), parl);
    return;
}
