...
REBOOL fetch_word(REBSER *cmds, REBCNT index, u32* words, REBCNT *cmd) {
	...
	return ((RXT_LIT_WORD == type || RXT_WORD == type) && (cmd[0] = RL_FIND_WORD(words, arg.int32a)));
}
...
REBOOL fetch_mode(REBSER *cmds, REBCNT index, REBCNT *result, REBCNT start, REBCNT max) {
	...
	if (RXT_WORD == type || RXT_LIT_WORD == type) {
		wrd = RL_FIND_WORD(agg_arg_words, arg.int32a) - start;
	} else if (RXT_INTEGER == type) wrd = (REBCNT)arg.int64;
	if (wrd >= 0 && wrd <= max) {
		result[0] = wrd;
		return TRUE;
	}
...
REBOOL fetch_color(REBSER *cmds, REBCNT index, REBCNT *cmd) {
	...
	if (RXT_WORD == type) {
		cmd[0] = RL_FIND_WORD(agg_arg_words, arg.int32a);
		return TRUE;
	}
	return FALSE;
}
