...
unless find system/modules 'agg [
	;if CI? [
...
foreach [file code] examples [
	...
	try/except [
		...
		if parse raw-src compose/deep [
			thru (probe join mold file " [") copy code-draw to "^/^-]"
			thru "precode:"
			opt [thru (probe join mold file " [") copy code-init to "^/^-]"]
			to end
		][
			replace/all code-draw "^-^-" "    "
			name: uppercase/part form file 1
			replace/all name #"-" #" "
		]
		...
		total-time: total-time + delta-time [img: agg/draw 480x480 code]
		save out img
